...
#include <homestore/homestore_defs.hpp>
...
static void init_homestore(std::string const &device_address) {
  // this should be static so that it stays in scope in the lambda in case
  // function ends before lambda completes
  ...
  params.init_done_cb = [&tl_init_lock = init_lock, &tl_wait_cv = wait_cv,
                         &tl_init_done =
                             init_done](std::error_condition,
...
int main(int argc, char *argv[]) {
  ...
  vol_params.size =
      SISL_OPTIONS["capacity"].as<uint32_t>() * Gi; /* assuming unit is Bytes */
  vol_params.uuid = boost::uuids::random_generator()();
  auto const vol_uuid = boost::uuids::to_string(vol_params.uuid);
  LOGINFO("Creating volume: name={}, uuid={}, size={}, page_size={}", vol_name,
          vol_uuid, vol_params.size, vol_params.page_size);
